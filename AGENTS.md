...
- **Gate only on permissions the server enforces**: `DEPOSIT`, `AJO`, `WITHDRAWAL`. Customers/accounts, EOD, remittance, dashboard, profile and notifications are baseline for every agent. `CUSTOMER_MANAGEMENT`, `LOAN_COLLECTION`, `SUPPORT_TICKETS`, `ACCOUNT_OPENING` grant nothing yet — gating on them locks agents out of things the server allows. See the note on `Permission` in `src/api/permissions.js`.

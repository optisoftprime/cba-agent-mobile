...
- **Money calls pass `{ confirmSession: true }`** (deposits, remittances, EOD submit). Finman refuses these for reasons unrelated to the session — a reconciliation hold, not enough cash — and uses 401 freely; ezone-agent-service relays and rewords them, so the status can't tell a refused ACTION from a dead SESSION. On a 401/403 the client makes one quiet `GET /agent/profile`: it answers → only the action was refused, the screen shows the message, the agent stays in; it is refused → sign out; unreachable → stay in. Signing out on a hold refusal would lock the agent out of the remittance and EOD that lift the hold. Any new call that moves money gets the flag.
- **EOD `status` is `Open`, `Balanced`, `Variance` or `Resolved`** (backend guide). `Resolved` = a supervisor settled a variance → closed, like Balanced, with its own wording.
- **The APP enforces permissions; the server does not.** Verified live with `CUSTOMER_MANAGEMENT`, `SUPPORT_TICKETS`, `DEPOSIT` and `AJO` revoked: `/agent/customers`, `/agent/support/tickets`, `/agent/loans` and the rest still answered 200. So a revoked feature stays reachable unless the app stops it, which is why gating is in TWO places: the entry point (faded control + `press` → modal: dashboard actions, and the tab itself via `TAB_PERMISSION` in `bottom-nav-bar`) and **the destination screen**, which renders `components/layout/locked-screen` when the permission is missing. The screen check is the enforcement — a notification tap, a deep link, or a screen already on the stack when an administrator revokes access never passes an entry point. Gated: customers, loans, support/tickets, deposit (every step before posting), ajo. Baseline, never gated: dashboard, profile, notifications, EOD, remittance, and the deposit success/receipt screens (money already posted — hiding a receipt helps nobody). An earlier note here said not to gate the codes the backend guide calls "grant nothing yet"; that was wrong — administrators do revoke them and expect the app to obey.
...
### Cash-ledger rules (backend guide, 2026-09-24)
- **Two decimals, everywhere money is typed.** A finer figure is a 400 ("Amount cannot be finer than one kobo"), and there is no coin below the kobo. `ui/amount-field` already caps input at two decimals — that is why every amount goes through it, including EOD's `countedCash`.
- **A movement's status is its status NOW, not at capture.** `Pending → Posted` or `Declined` once the Request Center decides, and `Posted → Reversed` if unwound. `Declined` is in `COLLECTION_STATUS_TONE`. Never cache a status or carry the capture response's one into a list — re-read.
- **Never compute headroom from `cashInHand`.** Cash awaiting approval to leave (a remittance, an escalated withdrawal) no longer counts as held, so a refusal can say "you are holding X" while a larger `cashInHand` is on screen. X is the net figure; show the server's message verbatim and do not do arithmetic with it.
- **`cashInHand` can be negative** — the till has gone positive, an invalid state, and it must be SHOWN, never floored at zero. `formatCurrency*` put the sign before the symbol so it reads as a figure.
- `GET /eod/current` is recomputed per read (an approval or reversal after a count changes the variance), so `eodCurrentQuery` sets `staleTime: 0`.
- A remittance retry with the same `clientReference` returns the original with `duplicate: true` *before* the cash check — so a retry is never refused as "not holding enough" by its own first attempt. Treat it as success, exactly like a deposit.
- A quiet day no longer raises a hold; `holdReason` says which case it is and is rendered verbatim.
- **Finman's own `/device/activate` is now public (agentCode + activationCode, no token) and `/me` reports a `state`** — but ezone-agent-service exposes NEITHER. The BFF still has `device-activation/{activate,resend-otp,verify-otp}` and `/agent/profile`, which is what this app uses. Don't port the finman flow until the BFF does.
- `LOAN_COLLECTION`, `SUPPORT_TICKETS` and `ACCOUNT_OPENING` still grant nothing server-side, but the app enforces them anyway — see the permissions note above; administrators revoke them and expect the app to obey.
...
## Rebranding — one file and one command
**The logo is eZONE; the app is CBA-Agent. That pairing is DELIBERATE** — the mark says which family the app belongs to, the name says which app it is. So the receipt reads "Generated by CBA-Agent" under an eZONE lockup, and the home-screen label stays CBA-Agent. Don't "fix" it.

`src/theme/brand.js` names the logo (`appName`, `logo: { light, dark }`) and every screen that shows it reads from there; `app.json` names the icon and splash images. Nothing else refers to either.
